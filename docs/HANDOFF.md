# HANDOFF — สถานะโปรเจกต์ ณ 2026-08-30 (อัปเดตหลังปรับ rewardRatio เป็น 3.0)
...
**เปลี่ยนล่าสุด (migration 0019 + 0020):** `rewardRatio` ขยับ 2.0 → **3.0** ทั้ง 4 กฎแล้ว ·
`outcome-notify` redeploy เป็น version 4 (ข้อ 7.3 ปิด) · คิวรีที่เคยต้องจำว่า
"ต้องกลับมารันเมื่อไม้ครบ" กลายเป็น view สองตัวแล้ว (`settings_effect`, `price_action_edge`)
โชว์อยู่บนหน้า `/stats`
...
| pg_cron | `evaluate-outcomes` ทุกนาที · `nightly-standing-experiment` 21:00 UTC (04:00 ไทย) — variants ล้อม 3.0 แล้ว (`reward 2 / 2.5 / 4`) |
