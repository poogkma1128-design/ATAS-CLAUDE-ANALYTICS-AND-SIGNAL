...
### 5.27 H4 — Mathematical Candle Signature × บริบท (ตรึงไว้ **ก่อน** เห็นผล V2)

**แผนเต็มอยู่ที่ `docs/experiments/2026-09-03-mathematical-candle-signature.md`** ·
Gate 0 query ที่รันซ้ำได้อยู่ที่ `docs/queries/h4_candle_signature_gate0.sql` ·
หัวข้อนี้เก็บเฉพาะสิ่งที่ต้องรู้เพื่อรับช่วงงาน
...
| V | **H4 — Mathematical Candle Signature × บริบท** | **ตรึงแผนแล้ว Gate 0 ผ่านแล้ว ยังไม่รัน V2** — §5.27 · `docs/experiments/2026-09-03-mathematical-candle-signature.md` · **ตัวเดียวในรายการนี้ที่ไม่ถูกบล็อกด้วย 0034** เพราะวัดบน `bars` (5,796 บาร์) ไม่ใช่ `opportunity_results` และเป็น `SELECT` ล้วน · V1 พบ 33.6% vs baseline 49.4% ชี้ทาง exhaustion **แต่ยังเป็น measurement artifact จนกว่า V2 จะวัดจาก `next_open`** · 🔒 Gate 0 ตรึงแล้ว: pooled เท่านั้น (ช่องเล็กสุด 15 บาร์), บริบท 1 มิติ, order flow รอ V3, `cum_delta` NULL 100% ⇒ Cumulative Delta บล็อก · งบทดสอบ 12 ตัว p<0.0042 · OOS เป็น forward test เปิด 2026-09-11 **ห้ามแอบดูก่อน** · ยังไม่มี independent reviewer |
