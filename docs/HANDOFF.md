...
**เปลี่ยนล่าสุด — กวาด trail จนสุดทาง 12 ค่า แล้ว *ไม่รับอะไรเลย* (ข้อ 5.4c):**
...
สามตัวนี้ยังไม่เคยถูกกวาดค่าแบบข้อ 5.11 — ค่าที่เห็นคือจุดตั้งต้น ไม่ใช่ค่าที่วัดมา
...
**ก่อนอื่น: อย่าเพิ่งเขียนตัวที่ 4** สามตัวแรกยังไม่มีตัวเลขสักตัว ถ้าเขียนต่อไปเรื่อย ๆ
จะได้กฎ 10 ตัวที่ไม่มีตัวไหนพิสูจน์ได้ ซึ่งแย่กว่ามี 4 ตัวที่รู้จัก
...
|---|---|---|---|
| 4 | Large / Block Trades | Time & Sales | **ยังไม่มีข้อมูล** — `bars.trades` เป็นจำนวนนับ ไม่ใช่ size รายไม้ ต้องแก้ indicator ให้ส่ง trade รายตัวหรือ histogram ของ size |
...
| 6 | Speed of Tape | timestamp เดิม | `bars.trades` มีอยู่แล้ว แต่ `HistoryBar` ไม่ได้พามาด้วย ต้องเพิ่ม 1 ฟิลด์ใน `types.ts` + select ใน `loadHistory()` และ `backtest.ts` — แก้เล็ก **ไม่ต้องแตะ indicator** ตัวที่ถูกที่สุดในกลุ่มนี้ |
...
supabase/functions/
  ...
  _shared/
    ...
    rules/           7 กฎ + registry (3 ตัวหลังปิดเสียงอยู่ — ข้อ 5.15)
