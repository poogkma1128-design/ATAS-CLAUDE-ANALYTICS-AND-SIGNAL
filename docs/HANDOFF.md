...
### 5.21 ใครรับผิดชอบอะไร และสิทธิ์แย้งเมื่อสั่งงานผิดคน

**แก้ 2026-09-02** — ฉบับเดิมแบ่งงานด้วย *ชื่อโมเดล* (GPT ทำโค้ด · Claude ตั้งสมมติฐาน)
ซึ่งพังในทางปฏิบัติ: §5.18a ข้อ 3 พบว่ามีรันที่ให้ผลดีกว่าค่าปัจจุบันอยู่ในฐานข้อมูลแต่ไม่ได้ขึ้นรายงาน
และผู้ตรวจจับได้ก็ต่อเมื่อ **query เอง** ไม่ใช่ตอนอ่านรายงาน — **ผู้ตรวจที่อ่านได้แค่รายงาน
จับ selection bias ไม่ได้ตามนิยาม เพราะสิ่งที่ถูกคัดออกไม่อยู่ในรายงาน**
นี่ไม่ใช่ความผิดของโมเดลไหน มันคือการบีบอัดตามธรรมชาติของการเขียนสรุป ใครเขียนก็เป็น

**หลักตั้งต้นที่ไม่เปลี่ยน:** GPT/Claude ไม่ใช่แหล่งความจริงของการคำนวณหรือ edge ของตลาด
ค่า R, win rate, drawdown, fill rate, sample size และ calibration ต้องมาจาก SQL/TypeScript
ที่ทำซ้ำได้พร้อม query/test ที่ตรวจย้อนกลับได้เสมอ **คำตอบที่ไพเราะของ AI ไม่ใช่หลักฐาน**
และ AI ต้องตอบ `ข้อมูลไม่พอ` เมื่อยังพิสูจน์ไม่ได้

#### แบ่งตามบทบาท ไม่ใช่ตามชื่อโมเดล

```
Proposer  →  Executor / Recorder  →  Independent Reviewer  →  Owner
(เสนอ)        (รัน + บันทึก)          (ตรวจอิสระ)            (อนุมัติ)
```

**กฎเหล็กสองข้อ:**

1. **ผู้เสนอไม่ตัดสิน hypothesis ของตัวเอง** — ไม่ว่าจะโมเดลไหน ถ้าเสนอในรอบที่ 1 ห้ามเป็นคน
   บอกในรอบที่ 3 ว่ามันได้ผล เพราะจะมี motivated reasoning โดยไม่รู้ตัว (อาการที่เห็นใน §5.18 คือ
   เข้มตอนปฏิเสธ แต่ผ่อนตอนยืนยันค่าเดิมและตอนเสนอให้ปิด long)
...
โมเดลไหนสวมบทบาทไหนก็ได้ **ยกเว้นสวมสองบทบาทในงานเดียวกัน** ตอนนี้โดยปกติคือ
GPT/Codex = Executor/Recorder (มีเครื่องมือ deploy) · Claude = Proposer หรือ Reviewer (เลือกอย่างเดียวต่องาน)

#### เจ้าภาพรายงาน — ทุกงานในระบบนี้

| งาน | เจ้าภาพ | ห้ามให้ใครทำ | ส่งมอบอะไร |
|---|---|---|---|
| คำนวณ R / WR / drawdown / fill rate / sample size | **SQL + deterministic code เท่านั้น** | **ห้าม AI คำนวณจากการอ่านตารางหรือ prompt** | query + เวอร์ชันข้อมูล + ผลที่รันซ้ำได้ |
| **Gate 0** — เกณฑ์ผูกจริงไหม แปลเหมือนกันทุก instrument ไหม | SQL + Reviewer | ห้ามข้าม **ต้องทำก่อนกวาดค่าเสมอ** | pass-rate conditional + marginal แยก instrument × direction × session · distribution · null rate · หน่วย · sensitivity |
| ตั้ง hypothesis + หา feature/กลไก | Proposer (ปกติ Claude) | ผู้เสนอห้ามตัดสินผลของมันเอง | **ไม่เกิน 3 ข้อต่อชุดข้อมูล** (ไม่ใช่ต่อรอบสนทนา — เหตุผลคือ multiple comparisons) · ระบุ entry/exit/invalidation, failure mode และ **เกณฑ์ที่ทำให้มันแพ้ บันทึกก่อนเห็นผล** |
| เขียนโค้ด · migration · ingest · dashboard · test · deploy | Executor (ปกติ GPT/Codex) | ห้ามอนุมัติผลของตัวเอง | diff ที่ review ได้ · build/test · deploy verification 3 ชั้น · rollback path |
| รันการทดลอง + บันทึกผล | Executor/Recorder | ห้ามเลือกว่ารันไหนจะขึ้นรายงาน | **`experiment_id` ครบทุกรัน รวม failed / omitted / superseded** + variant ที่วางแผนแล้วไม่ได้รัน |
| เขียน §5.x / Handoff | Recorder | — | ต้องมี experiment_id, drawdown, fill rate, ด่าน #18 รายตัว และบรรทัด "ไม่ได้รัน/ไม่ได้รายงาน" (ถ้าไม่มีให้เขียนว่า "ไม่มี") |
| ท้าทายผล หา look-ahead / leakage / selection bias / data-snooping / regime dependence | **Independent Reviewer** (อิสระจากผู้เสนอและผู้รัน) | ห้ามเป็นคนเดียวกับ Proposer ของ hypothesis นั้น | ต้อง query ดิบเอง · แนบ SQL ของทุกตัวเลขที่อ้าง · และ **ระบุว่าอะไรจะทำให้ข้อท้าทายของตัวเองผิด** |
| ตัดสินใจ deploy / เปิด-ปิด Telegram / เปิด filter / เงินจริง | **เจ้าของ จากหลักฐาน** | **AI ไม่มีสิทธิ์อนุมัติเอง ทุกกรณี** | ต้องผ่าน gate ด้านล่าง + คำสั่งชัดเจน + บันทึกใน PR/Handoff |
| ตั้งค่า ATAS / ติดตั้ง DLL / Supabase Auth / revoke secret | **เจ้าของ** (AI ไม่มีสิทธิ์เข้าถึง) | — | ดู §7.1 และ §0G.1 |

#### Gate ที่ใช้ตัดสินแทนความเห็นของ AI

1. hypothesis และเกณฑ์ผ่าน/แพ้ถูกบันทึก **ก่อน**เห็นผล · ห้ามปรับแล้ววัดชุดเดิมซ้ำจนดูดี
   (การต่อกริดหลังเห็นผลทำได้ถ้าใช้ไปในทางปฏิเสธ **แต่ต้องติดป้ายว่า post-hoc** — ดู §5.18a ข้อ 8)
...
5. **การทดสอบความแตกต่างต้องใช้ block bootstrap ตาม session × instrument ไม่ใช่ paired t-test**
   หน่วยสุ่มคือเซสชัน ไม่ใช่ไม้ · และ estimand คือ **policy contrast** (ชุดไม้ทั้งชุดที่ค่า A ผลิต
   เทียบชุดที่ค่า B ผลิต) ไม่ใช่ผลต่างบนไม้ที่ซ้ำกัน — การจับคู่เฉพาะ intersection สร้าง
   selection bias ใหม่ เพราะไม้ที่อยู่ arm เดียวคือไม้ที่การเปลี่ยนค่าพูดถึงพอดี
   **ยังทำไม่ได้จนกว่า §7.2-O2 จะเสร็จ** ⇒ ระหว่างนี้ห้ามอ้างว่าความต่างมีนัยสำคัญ
6. `confidence_v2` อยู่ shadow ต่อไปจนกว่าโมเดลที่ตรึง version แล้วชนะ baseline บนข้อมูล forward
   ที่โมเดลไม่เคยเห็น และเจ้าของอนุมัติเป็นลายลักษณ์อักษร

#### ระดับการแย้ง — AI ต้องแจ้งก่อนทำ ไม่ใช่ทำตามเงียบ ๆ

| ระดับ | เมื่อพบการสั่งงานผิดบทบาท / ข้ามหลักฐาน | AI ต้องทำอะไร | ผลเสียหากฝืนทำ |
|---|---|---|---|
| **L1 — แจ้งเตือน** | ขอให้สรุปหรือคำนวณจากข้อมูลที่ query ได้ แต่ไม่มี citation/query แนบ | ทำได้เฉพาะหลังแนบ query/source และบอกข้อจำกัด | รายงานคลาดเคลื่อน ใช้ประกอบการตัดสินใจผิดได้ แต่ยังไม่เปลี่ยนระบบ |
| **L2 — ต้องทบทวน** | ให้ Executor ยืนยัน edge จาก narrative · ให้ Proposer ตรวจงานตัวเอง · เชื่อ backtest รอบเดียว · กวาดค่าโดยข้าม Gate 0 | หยุดไว้ที่ draft/experiment · ขอ hypothesis, baseline, Gate 0 และ holdout plan | overfitting และ data-snooping · เสียเวลา เกิดความมั่นใจปลอม และทำให้การทดลองถัดไปตีความยาก |
| **L3 — ห้ามดำเนินการจนมีหลักฐาน** | เปิด rule/filter/Telegram จาก backtest หรือ confidence ที่ยังไม่ผ่าน forward · ปรับ threshold ระหว่างดูผล · มอบ deploy/migration โดยไม่มี verification owner | ปฏิเสธการเปลี่ยน production · เสนอขั้น shadow/peer review/rollback | ขาดทุนหรือพลาดโอกาสจริง · spam Telegram · ทำลาย baseline และปนเปื้อนข้อมูลทดลองจนใช้ตัดสินต่อไม่ได้ |
| **L4 — หยุดและขออนุมัติชัดเจน** | สั่งให้ AI ส่งคำสั่งซื้อขายจริง · ปิด safety/telemetry · ลบหรือแก้ข้อมูล history · rotate secret โดยไม่มีแผน · ข้าม test/deploy verification | ไม่ทำ · สรุป impact, rollback และสิทธิ์ที่ต้องใช้ให้เจ้าของตัดสินใจ | ความเสียหายเงินจริง/การควบคุมความเสี่ยง · สูญเสียประวัติพิสูจน์ผล · credential exposure หรือระบบรับสัญญาณหยุดทำงาน |
...
ผู้ใช้ยัง override ได้ แต่ **L3/L4 ต้องบันทึกเหตุผล, scope, owner, rollback และคำยืนยันชัดเจน
ใน PR/Handoff ก่อนทำ** · ไม่มีคำว่า "user ขอแล้ว" ที่ทำให้ผลทดลองซึ่งเสียความน่าเชื่อถือกลับมาเชื่อถือได้

#### ถ้าเจ้าของสั่งผิด AI — ตัวอย่างและสิ่งที่ AI ต้องตอบกลับ

| สั่งแบบนี้ | ผิดตรงไหน | ระดับ | AI ต้องตอบว่า |
|---|---|---|---|
| "ดูตารางนี้แล้วบอกหน่อยว่ากฎไหนกำไรดีสุด" | ให้ AI เป็นเครื่องคิดเลข | **L1** | ขอรัน query ก่อน แล้วตอบพร้อม SQL — ไม่ตอบจากการอ่าน |
| "Claude คิด feature มาแล้วบอกเลยว่าอันไหนดีสุด" | Proposer ตัดสินตัวเอง | **L2** | เสนอได้ 3 ข้อพร้อมเงื่อนไขแพ้ **แต่ให้คนอื่นตัดสิน** |
| "GPT กวาดค่าใหม่ให้หน่อย" (ยังไม่ทำ Gate 0) | กวาดพารามิเตอร์ที่อาจผูกไม่ได้ | **L2** | ขอ Gate 0 ก่อน มิฉะนั้นเผา CPU quota (§3.11) ฟรี |
| "ผลดูดี เปิด Telegram กฎนี้เลย" | เปิดจาก backtest ไม่มี forward | **L3** | ปฏิเสธ · เสนอ shadow + forward + owner approval เป็นลายลักษณ์อักษร |
| "เอา confidence มากรองสัญญาณ" | v2 ยัง `score:null` และ v1 วัดแล้วไม่มีข้อมูล (§5.19) | **L3** | ปฏิเสธ · อ้าง §5.19 corr 0.013–0.051 |
| "AI ยิงออเดอร์ให้เลย" / "หมุน secret ให้ที" | เกินขอบเขต AI | **L4** | ไม่ทำ · สรุป impact + rollback + สิทธิ์ที่ต้องใช้ ให้เจ้าของทำเอง |
| "ตั้ง `announcement_mode = manual` เพื่อให้สัญญาณมาไว ๆ" | ข้าม evidence gate | **L3** | ปฏิเสธ · §0A.1 ข้อ 3 ระบุว่าเป็น owner override ที่ต้องมีเหตุผลบันทึก |

#### ตัวอย่างการจับคู่คำสั่งให้ถูกบทบาท

- "คิด feature ที่อาจแยก long/short ได้ และเขียนว่ามันแพ้อย่างไร" → **Proposer** · ผลเป็น hypothesis ไม่ใช่กฎใหม่
- "ทำ query/feature snapshot/test/migration และตรวจ ingest ที่ production" → **Executor** · พร้อม verification ตาม §5.20 และ §7.4
- "ตรวจว่าหัวข้อนี้ลำเอียงไหม" → **Independent Reviewer** · ต้อง query ดิบเอง แนบ SQL และบอกว่าอะไรจะทำให้ตัวเองผิด
- "คะแนนนี้แม่นไหม / เปิด Telegram ได้ไหม" → **SQL + forward result + เจ้าของ** · ถ้ายังไม่มี cohort ปิดผล คำตอบเดียวคือ **ยังตัดสินไม่ได้**
