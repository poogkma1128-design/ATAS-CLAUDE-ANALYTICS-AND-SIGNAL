...
## 0. เริ่มที่นี่ — สถานะ ณ 2026-08-30 03:20 UTC

**ทุกอย่างที่สั่งไว้ทำเสร็จและขึ้น production แล้ว** ไม่มีงานค้างกลางทาง

**สถานะ feed ตอนนี้:** BTCUSDT = **สด** (467 แท่ง แท่งล่าสุด 8 นาทีที่แล้ว) ·
...
### สามเรื่องที่รอ "เจ้าของตัดสิน" ไม่ใช่รอโค้ด

1. **ขยับ `rewardRatio` 2.0 → 3.0 ไหม** — วัดครบแล้ว ดีขึ้นทั้ง 4 instrument บนไม้ชุดเดียวกัน
   174 ไม้ (ข้อ 5.6) แก้ที่ `/rules` ใช้เวลา 10 วินาที ถ้าแย่ลงกดย้อนที่ `/experiments`
2. **เปิดชาร์ต MNQ/NQ/GC ค้างไว้ตอนตลาดเปิด** — ตอนนี้ส่งได้ปกติแล้ว แต่ตลาดปิดอยู่
   ต้องได้อีก 2 เซสชันถึงจะตัดสินชะตา MNQU6 ได้ (ข้อ 5.8)
3. **แก้ Supabase Auth ให้ล็อกอินเว็บได้** (ข้อ 7.1 #2 และ #3) — ตอนนี้หน้า `/experiments`
   ที่เพิ่งทำเสร็จ **เจ้าของยังเปิดดูไม่ได้** เพราะติดล็อกอิน นี่คืออุปสรรคใหญ่สุดตอนนี้
...
หลัง migration 0014 (trail 1/0.5 → 0.5/0.25 เมื่อ 2026-08-29 18:50 UTC) เทียบไม้ที่ resolved แล้ว:

| | ก่อน (275 ไม้) | หลัง (23 ไม้) |
|---|---|---|
| SL | 130 (47%) | 12 (52%) |
| TP | 51 (19%) | 1 (4%) |
| **trail** | 73 (27%) | **10 (43%)** |
...
**สิ่งที่ทำนายไว้เกิดขึ้นจริง:** สัดส่วน trail ขึ้นจาก 27% เป็น 43% TP ร่วงจาก 19% เหลือ 4%
และ timeout หายไปหมด ตรงกับที่จำลองไว้ (70 trail / 132 stop → 106 trail / 113 stop)

**แต่ R ยังตามไม่ทัน** — ยังไม่ถึงเกณฑ์ 30 ไม้ที่ตั้งไว้ และ 23 ไม้นี้เป็น BTCUSDT
ตัวเดียวช่วงกลางคืน ไม่ใช่สองสินทรัพย์แบบที่การจำลองครอบคลุม **ยังไม่สรุปและยังไม่ถอย**
รันคิวรีนี้ซ้ำเมื่อไม้ครบ 30:

```sql
select case when s.fired_at > timestamptz '2026-08-29 18:50:00+00' then 'after' else 'before' end as era,
       o.exit_reason, count(*) as trades,
       round(sum(o.pnl_ticks/nullif(s.risk_ticks,0))::numeric,2) as total_r
  from public.signals s join public.signal_outcomes o on o.signal_id = s.id
 where o.status='resolved' group by 1,2 order by 1,2;
```

ถ้า R ยังติดลบเมื่อครบ 30 ไม้ **และ** มีสองสินทรัพย์ขึ้นไป ให้กดย้อนกลับที่ `/experiments`
