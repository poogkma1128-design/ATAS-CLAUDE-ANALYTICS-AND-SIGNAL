...
**เปลี่ยนล่าสุด (migration 0024 + 0025, `backtest` v3):** ตัวรัน backtest **บันทึก drawdown
...
| Edge function `ingest` | **version 11, ACTIVE** (`verify_jwt: false` — auth ด้วย INGEST_TOKEN เอง) — เลื่อนจาก repo แล้วแต่ผลลัพธ์เท่าเดิม ดูข้อ 7.4 |
...
| Edge function `outcome-notify` | **version 4, ACTIVE** — โค้ดตรงกับ repo แล้ว (เคยเป็น v3 ที่ bundle ตัวเก่า ดูข้อ 7.3) |
...
### 7.4 `ingest` v11 เลื่อนจาก repo แล้ว — และเหตุผลที่**ยังไม่ควร** deploy

`_shared/plan.ts` เปลี่ยนไปแล้ว (เพิ่ม `pullbackShare`) และ `ingest` bundle ไฟล์นี้อยู่
แต่ค่าเริ่มต้นคือ 0 → `entry = close − (close − anchor) × 0 = close` **ผลลัพธ์เหมือนเดิมทุกตัวอักษร**

deploy `ingest` ต้องอัปโหลด 16 ไฟล์ด้วยมือ และมันคือ**เส้นทางสด** — ถ้าพลาดไปไฟล์เดียว
กระทบไม้จริงที่ยิงเข้าโทรศัพท์ ความเสี่ยงจริงแลกกับกำไรศูนย์ในวันนี้ **จึงเลือกไม่ deploy**

**เมื่อไรถึงต้อง deploy จริง ๆ:**
- เมื่อจะแก้อะไรใน `_shared/` ที่**เปลี่ยนผลลัพธ์** ของเส้นทางสด (กฎ, `plan.ts` ที่มีผลจริง, `telegram.ts`)
- ไม่ใช่เมื่อจะเปิด `pullbackShare` — เพราะ**ไม่รับมาใช้แล้ว** (ข้อ 5.13) และถ้าวันหนึ่งจะเปิดจริง
  ตัวที่ต้องแก้ก่อนคือ `evaluate_pending_outcomes()` ฝั่ง SQL ไม่ใช่ deploy อย่างเดียว (ข้อห้าม #19)

ตรวจว่าเลื่อนไปแค่ไหน: `git log --oneline -- supabase/functions/_shared/` เทียบกับ
`updated_at` ของ `ingest` ใน `list_edge_functions`
