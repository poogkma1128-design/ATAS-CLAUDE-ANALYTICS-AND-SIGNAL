...
### 5.24 H1 — เทรนเป็น "ตัวกรอง" ไม่ใช่กฎใหม่ (ตรึงไว้ **ก่อน** เห็นผล)
...
| 6 | **เช็กความลึกของข้อมูล 5m OHLCV บน ATAS** — เลื่อนกราฟ 5m ย้อนหลังทีละ instrument (GC, NQU6, MNQU6, BTCUSDT) ดูว่าไปได้ไกลสุดกี่วัน/เดือน ก่อนตัดสินใจสร้าง deep-backfill pipeline สำหรับ H4 V3.1 | ต้องเห็นหน้าจอ ATAS บนเครื่อง Windows — ดูบริบทที่ `docs/experiments/2026-09-03-candle-signature-v3.md` §ญ.11 |
...
| AA | **H4 V3.1 — Deep historical backfill จาก ATAS โดยตรง** | **เสนอ 2026-09-03 · บล็อกรอเจ้าของเช็ก ATAS** — §ญ.11 ของ `2026-09-03-candle-signature-v3.md` · แทร็กคู่ขนานกับ V4 (เพิ่ม n เร็วกว่ารอ live feed) ไม่ใช่ตัวแทน · 🔴 **ตรวจโค้ดแล้ว: `SendBackfill()` ปัจจุบันเพดาน 200 แท่ง (~16.7 ชม.) และส่งทีเดียวไม่แบ่งชุด** (`SignalBridgeIndicator.cs:75-78`, `ingest.ts:20`) ⇒ ดึงหลายเดือนไม่ได้ด้วยโค้ดปัจจุบัน ต้องเขียนใหม่ · ✅ OHLCV ไม่ต้องพึ่ง footprint (`Volume`/`Delta` มาจาก `GetCandle()` ตรง ๆ) ⇒ ข้อจำกัด DOM/footprint ของ ATAS Replay ไม่กระทบ Backtest A · 🔴 **ห้าม backfill ผ่าน `ingest` ปกติเด็ดขาด** — จะรัน rule evaluation ย้อนหลังด้วย param วันนี้แล้วปนเปื้อน `public.signals`/`signal_outcomes` ซึ่งทุกสถิติที่มีอยู่ (§5.18, ประชากร H1/H2/H3, cohort confidence_v2) สมมติว่าเป็นประวัติ live จริง · ทางปลอดภัยคือตาราง+endpoint แยกใหม่ (ออกแบบไว้ ยังไม่เขียน) ต้องผ่าน migration+Gate 0+Independent Review เหมือนเดิม · เงื่อนไขถัดไป: ดู §7.1 ข้อ 6 |
