...
### 0Z.5 ยังไม่ได้ทำ — ต้องทำตามลำดับนี้เท่านั้น

1. **deploy `ingest` ก่อน** ⇒ ถ้าแก้ฐานข้อมูลก่อน deploy ค่าจะถูกเขียนทับกลับทันที
2. แก้สี่แถว: MNQU6→0.25 · GC→0.10 · BTCUSDT→0.10 · NQU6 คงเดิม + เติม `tick_value`
3. ตัดสินว่าจะทำอย่างไรกับ 3,331 แถวที่ derive ไว้แล้ว (คำนวณทับ vs เก็บของเดิมไว้ด้วย)

รอบนี้ **ยังไม่ deploy และไม่เขียนฐานข้อมูล** — ทุกคำสั่งกับ project จริงเป็น `SELECT` ·
rollback ของ commit นี้ = revert (ยังไม่มี state ในฐานข้อมูลให้ย้อน)
