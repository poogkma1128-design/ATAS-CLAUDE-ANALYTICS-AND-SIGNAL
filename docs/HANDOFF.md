...
## 0AE. หาเจอแล้วว่าทำไม MNQ Pullback ยิงศูนย์ — **ไม่ใช่ตัวกรองเข้ม ไม่ใช่หน้าต่างข้อมูล** (2026-09-07)
...
บนแท่ง MNQU6 5m ชุดเดียวกัน **2,434 แท่ง** (2026-04-15 → 2026-09-07):

| ตัววัด | ผล |
|---|---|
| โอกาสที่ evaluator เต็ม (6 แท่ง) เปิดได้ | **42** |
| ในนั้นมี trigger ยืนยัน | **13** |
| ที่ adapter live ปัจจุบันยิงได้จริง | **6** |

**สาเหตุคือ `touch_bar_only`** — adapter ยิงได้เฉพาะเมื่อ "แตะระดับ" กับ "ยืนยันด้วย order flow"
เกิดในแท่งปิดเดียวกัน. ราวครึ่งหนึ่งของการยืนยันมาถึงในแท่ง**ถัดไป** และ edge function จำสถานะ
ข้ามการเรียกไม่ได้ setup จึงอยู่รอครบ 6 แท่งตามสัญญาไม่ได้ ⇒ หายไปเลย. นี่ไม่ใช่เรื่อง threshold.

โอกาสทั้ง 42 ตกอยู่บน 5 วันเทรด (31 ส.ค. – 4 ก.ย.) จาก 7 วันที่ฟีดเต็ม ⇒ ความถี่ระดับ
**ใช้งานได้จริง** แต่ **ยังไม่มีการคำนวณกำไร/ขาดทุนใด ๆ ในรอบนี้** ห้ามอ้างตัวเลข R จากเอกสารนี้.

### 0AE.2 ข้อสรุปเดิมที่ต้องถอน
...
### 0AE.3 การผ่อนตัวกรองซื้ออะไรได้บ้าง (วัดแล้ว)

| zoneProximity | โอกาส | ยืนยัน | live ยิงได้ |
|---|---|---|---|
| 0.25 | 30 | 8 | 3 |
| **0.50 (ใช้อยู่)** | **42** | **13** | **6** |
| 0.75 | 54 | 15 | 7 |
| 1.00 | 65 | 17 | 8 |
| 1.50 | 78 | 21 | 9 |

ขยายโซน 3 เท่า ได้ live เพิ่มจาก 6 → 9. **เก็บสถานะ setup ข้ามแท่งที่โซนเดิม ได้ 6 → 13.**
⇒ ลำดับความสำคัญคือ *สถานะ* ไม่ใช่ *ผ่อนเกณฑ์* และการเก็บสถานะไม่ได้ขยายเงื่อนไขเข้าไม้เลย.

### 0AE.4 ปัญหาข้อมูลที่เจอระหว่างทาง (ยังไม่แก้ ยังไม่แตะ)

1. **ฟีด `5m` ไม่ได้เป็น 5 นาทีทั้งหมด** — แท่ง MNQU6 ตั้งแต่ 2026-08-27 จำนวน 2,353 แท่ง มี **567 แท่ง (24%)**
   ที่นาทีเปิดไม่ลงตัวด้วย 5 และมี 199 คู่ที่ห่างกัน **1 นาที**; วันที่ 2026-09-03 มี 686 แท่ง และ 2026-08-31 มี 490 แท่ง
   ทั้งที่วันเทรด 23 ชม. มีได้มากสุด 276 ⇒ คือการปนเปื้อน timeframe ตาม §0L **ที่ยังเกิดอยู่**
   และมันไหลเข้า median true range ซึ่งเป็นหน่วยวัดระยะทางของกลยุทธ์นี้ทั้งหมด
2. **footprint reconcile ได้ 125 จาก 237 แท่ง (52.7%)** ในช่วง setup ⇒ ตัวเลข trigger เป็น **ขอบล่าง**
3. **328 คู่ (แท่ง, anchor) ปิดด้วย `previous_day_completeness`** และ **514 ปิดด้วย `insufficient_history`**
   ⇒ ระบบแยก “ข้อมูลขาด” ออกจาก “ไม่มี setup” ได้ถูกต้องตาม §0AB.3

### 0AE.5 สถานะการ deploy และงานค้าง

- **ยังไม่ deploy อะไรเลยในรอบนี้**: `ingest` ยังเป็น v19, `backtest` ยังเป็น v9, migration 0039 ไม่แตะ,
...
- **ยังไม่มี independent review** ของผลรอบนี้ ⇒ ห้ามใช้เป็นเหตุผลเปลี่ยน runtime จนกว่าจะมี
- **การตัดสินที่รอเจ้าของ (L3):** จะทำ durable setup state หรือไม่ — คือแถวถาวรที่ผูกกับ
  `(กลยุทธ์, instrument, ทิศ, ระดับ, ครั้งที่แตะ)` ที่ ingest เปิดตอนแตะ และปิดเมื่อ trigger / invalidate /
  ครบ 6 แท่ง. นี่คือสิ่งที่เปลี่ยน 6 เป็น 13 และเป็นการเปลี่ยนสิ่งที่ live ยิงจริง
- **ยังไม่ทำโดยตั้งใจ:** ไม่คำนวณผลหลังต้นทุน (ต้องแก้ข้อ 0AE.4.1 ก่อน) · ไม่แตะ tick/`tick_value` ·
  ไม่เปิดอีกสองกลยุทธ์ · ไม่ apply 0033–0038 · ไม่แตะ NQU6 shadow policy
