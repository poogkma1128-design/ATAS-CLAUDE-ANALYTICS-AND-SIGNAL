...
## 0. เริ่มที่นี่ — สถานะ ณ 2026-08-31

**ทุกอย่างที่สั่งไว้ทำเสร็จและขึ้น production แล้ว** ไม่มีงานค้างกลางทาง

**เปลี่ยนล่าสุด — Speed of Tape ขึ้นแล้ว และคอลัมน์ในเอกสารนี้เคยเขียนผิด (ข้อ 5.16):**
...
ต้องรอสัญญาณสดสะสมแล้วดูที่ `setup_stats_by_instrument` · และยังไม่มีสัญญาณสดสักตัว
(feed หยุดที่ 22:50 deploy 23:11) — `signals` จึงยังไม่มีแถว `speed_of_tape` เลย
แปลว่าเช็ก "announced = 0" ตอนนี้ผ่านแบบว่างเปล่า ไม่ใช่แบบพิสูจน์แล้ว **ต้องเช็กซ้ำเมื่อ feed กลับมา**
(ข้อ 7.4 layer 3 ก็ค้างด้วยเหตุผลเดียวกัน)
...
| L | **ตรวจ deploy ชั้น 3 ของ `ingest` v14** | ค้างอยู่ — feed หยุดที่ 22:50 ก่อน deploy 23:11 จึงยังไม่มี `ingest_log` แถวใหม่ให้ดู **ต้องเช็กเมื่อ feed กลับมา** พร้อมยืนยันว่า `signals` มีแถว `speed_of_tape` และ `announced = 0` (ข้อ 7.4) |
| M | **ตัดสินชะตา `speed_of_tape`** | ค้างอยู่ — ตัวเลข backtest ชุดแรก**ฝั่ง long ติดลบและขันเกลียวแล้วแย่ลง** (ข้อ 5.16) ถ้าสัญญาณสดยืนยัน ให้ปิดด้วย `rule_overrides` หรือปิดทั้งกฎ **ห้ามไล่ปรับ `minRateRatio` จนกว่าตัวเลขจะสวย** (ข้อห้าม #14) |
...
⚠️ **ชั้น 3 ยังตรวจไม่ได้** — ingest ล่าสุดคือ 22:50 ซึ่งอยู่**ก่อน** deploy
`ingest_log` จึงไม่มีแถวใหม่เลยหลัง 23:11:57 **ต้องกลับมาเช็กเมื่อ feed กลับมา**
พร้อมกับเช็กว่า `signals` มีแถว `speed_of_tape` และ `announced = 0` จริง
(ตอนนี้ผ่านแบบว่างเปล่า เพราะยังไม่มีสัญญาณสักตัว)
(error 13 แถว `JWT issued at future` ในหน้าต่างนั้นคือข้อ 3.12 และเกิดก่อน deploy ทั้งหมด)
...
**สถานะ: กำลังทำ** เขียนหัวข้อนี้ก่อนลงมือ เพื่อให้เซสชันใหม่ทำต่อได้โดยไม่ต้องไล่ประวัติเก่า
...
**วิธีแก้เล็กมาก:** เพิ่ม loop ที่สี่ใน `resultRows()` ตามรูปแบบของสอง loop ที่มีอยู่แล้วเป๊ะ ๆ
...
⚠️ MNQ/NQ/GC ตลาดปิด แท่งค้าง 28 ส.ค. ด่าน #18 จึงตัดสินจากตัวอย่างบางฝั่ง futures
