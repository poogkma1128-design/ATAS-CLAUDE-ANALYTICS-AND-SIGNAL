...
## 0. เริ่มที่นี่ — สถานะ ณ 2026-08-30 03:45 UTC

**ทุกอย่างที่สั่งไว้ทำเสร็จและขึ้น production แล้ว** ไม่มีงานค้างกลางทาง

**เปลี่ยนล่าสุด (migration 0022 + 0023):** วัด **drawdown** ได้แล้ว (ไม่เคยมีมาก่อนเลย) ·
...
### 5.13 pullback entry — วัดได้แล้ว แต่ยังปิดอยู่ และเปิดจาก `/rules` เฉย ๆ ไม่ได้
...
**นี่เป็นชั้นหลักฐานที่ดี** — ไม่ได้เปลี่ยนว่าไม้ไหนเกิด เปลี่ยนแค่ราคาเข้า จึง backtest บนไม้ชุดเดียวกันได้
เหมือน reward/trail ไม่ใช่เหมือน threshold ของกฎ

**กับดักที่ต้องนับ:** ไม้ที่ราคาไม่ย่อกลับมา = **ไม่ได้เข้าเลย** และไม้พวกนั้นมักเป็นไม้ที่จะกำไร
ถ้านับแต่ไม้ที่เข้าได้ ค่าที่แย่กว่าจะดูดีกว่า → `simulate()` คืน `{ trades, missed }` แล้ว
และ `fillIndex()` เช็กว่าราคาแตะจริงในกี่แท่ง (`pullbackWithinBars`)

**ห้ามเปิดจาก `/rules` โดยยังไม่แก้ตัวให้คะแนน** — `evaluate_pending_outcomes()` สมมติว่าทุกแผนได้เข้าที่แท่งสัญญาณ
ถ้าเปิด pullback ตอนนี้ ไม้ที่ไม่เคยได้เข้าจะถูกให้คะแนนเหมือนเข้าแล้ว **นี่คือ param ตัวแรกที่รับมาใช้
ไม่ได้ด้วยการแก้แถวใน `rules.params` อย่างเดียว** — ต่างจากทุกตัวก่อนหน้า
...
19. **อย่าเปิด `pullbackShare` จาก `/rules` โดยยังไม่แก้ `evaluate_pending_outcomes()`**
    ตัวให้คะแนนสมมติว่าทุกแผนได้เข้าที่แท่งสัญญาณ ไม้ที่ราคาไม่ย่อกลับมาจะถูกนับเหมือนเข้าแล้ว (ข้อ 5.13)
