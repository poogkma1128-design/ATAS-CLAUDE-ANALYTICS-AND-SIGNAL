...
> ⇒ ถ้ารอบ live เก็บแท่งลงฐานข้อมูลสำเร็จ **แต่ตายก่อนจะประเมินกฎ** (เช่น ฟังก์ชันหมดเวลา
> หรือการเขียน `strategy_setups` ล้ม) **การส่งแท่งนั้นซ้ำจะกู้คืนไม่ได้อีกแล้ว** — ทั้งตัวส่งซ้ำ
> และ `signals` ที่ unique บน `(bar_id, rule_key, direction)` ต่างก็ปฏิเสธมัน
>
...
> ต้องลบแถวใน `bars` ทิ้งก่อนแล้วค่อยส่งใหม่ ซึ่งเป็นงานที่ต้องให้เจ้าของตัดสิน ไม่ใช่ของอัตโนมัติ
