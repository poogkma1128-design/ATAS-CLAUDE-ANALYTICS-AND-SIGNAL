...
### ค่าเริ่มต้นของ `poc_shift` ถูกจูนจากข้อมูลจริงแล้ว

รอบแรกที่รันกับ MNQ 5 นาที กฎนี้ยิง 45 จาก 71 สัญญาณ คือเกือบครึ่งของทุกแท่ง
สาเหตุคือ `minTicks: 3` — ในเซสชันนั้น POC ขยับระหว่างแท่งข้างเคียง **มัธยฐาน 45 ticks**
เกณฑ์ 3 ticks จึงไม่ได้กรองอะไรเลย

สิ่งที่แยกเทรนด์ออกจาก chop จริง ๆ คือ **จำนวนก้าวที่ไปทางเดียวกันติดกัน** ไม่ใช่ระยะทาง:

| ตั้งค่า | แท่งที่เข้าเงื่อนไข (จาก 132 แท่ง) |
|---|---|
| `consecutive: 2` (ของเดิม) | 57 |
| `consecutive: 3` | 23 |
| `consecutive: 3` + เพิ่มเกณฑ์ระยะทางอีก | 22 |

ค่าเริ่มต้นจึงเปลี่ยนเป็น `{"minTicks": 8, "consecutive": 3, "hvnShare": 0.25}`
ถ้าอยากได้สัญญาณถี่ขึ้นหรือน้อยลง ปรับ `consecutive` เป็นตัวแรก — มันคือตัวที่มีผลจริง

### แท่งย้อนหลังจะไม่ถูกยิงเข้า Telegram

ตอน indicator เริ่มทำงาน มันส่งประวัติทั้งหมดที่เห็นบนชาร์ตมาใน request เดียว
ระบบดูเวลาปิดของแต่ละแท่งแยกกัน → **เก็บแท่งย้อนหลังลง database และนับในสถิติตามปกติ
แต่ไม่แจ้งเตือน** ส่วนแท่งที่เพิ่งปิดยังแจ้งได้แม้มากับ request หลายแท่ง. แท่งที่ฐานข้อมูลปิดแล้ว
จะไม่ถูก evaluate ซ้ำ จึงไม่สร้างสัญญาณย้อนหลังจากการ reload กราฟ.

> **ข้อแลกเปลี่ยนที่ต้องรู้:** กฎ "แท่งที่ปิดแล้วไม่ถูก evaluate ซ้ำ" ไม่มีข้อยกเว้น
> ⇒ ถ้ารอบ live เก็บแท่งลงฐานข้อมูลสำเร็จ **แต่ request หยุดก่อนประเมินกฎหรือก่อนเขียน signal**
> (เช่น ฟังก์ชันหมดเวลา) **การส่งแท่งนั้นซ้ำจะกู้คืนไม่ได้อีกแล้ว** เพราะรอบใหม่เห็นแท่งเดิมเป็น
> `is_closed` และข้ามก่อนเข้า evaluator. การเขียน `strategy_setups` ล้มไม่ใช่กรณีนี้: จุดนั้นอยู่หลัง
> evaluate และโค้ดตั้งใจคืน signal rows ต่อไปยัง `persistSignals()` แม้บันทึก carry ไม่สำเร็จ.
>
> ยอมแลกแบบนี้ตั้งใจ: การนับซ้ำจากการรีโหลดกราฟเกิดบ่อยและทำให้สถิติเพี้ยนแบบเงียบ ๆ
> ส่วนการประเมินพลาดเกิดยากและเห็นได้จาก log. ถ้าจำเป็นต้องประเมินแท่งเก่าใหม่จริง ๆ
> ต้องใช้ขั้นตอนซ่อมที่เจ้าของอนุมัติและตรวจผลเป็นรายกรณี; ระบบไม่มี recovery อัตโนมัติสำหรับกรณีนี้.
...
| ไม่มีข้อความเข้า Telegram | ดู `signals.telegram_status`: `skipped_*` บอกเหตุผลที่ตั้งใจไม่ส่ง, `failed` มีรายละเอียดใน `telegram_error`, `sent` มี `telegram_message_id`; `legacy_unknown` คือแถวก่อนเพิ่มสถานะนี้ |
