...
/**
 * The indicator authenticates with its own shared secret rather than a Supabase
 * JWT, so this function is deployed with verify_jwt disabled and checks itself.
 */
function authorized(req: Request): boolean {
  const expected = Deno.env.get("INGEST_TOKEN");
  if (!expected) {
    console.error("INGEST_TOKEN is not set; refusing all requests");
    return false;
  }
