-- Executable regression test for migration 0036.
-- Run after migrations 0001-0036 on a disposable PostgreSQL database:
--   psql "$TEST_DATABASE_URL" -v ON_ERROR_STOP=1 \
--     -f supabase/tests/0036_seal_counterfactual_artifacts_test.sql
...
values ('t0036_rule', '0036 regression fixture', 10);

insert into public.instruments (id, symbol, exchange, tick_size, tick_value)
values ('35000000-0000-0000-0000-000000000001', 'T0036', 'TEST', 0.25, 1);
...
insert into public.signals (
  ...
  ('35000000-0000-0000-0000-000000000011', 35001,
   '35000000-0000-0000-0000-000000000001', '5m', 't0036_rule', 'long', 103,
...
values ('35000000-0000-0000-0000-000000000021', '0036 sealed contract',
        array['T0036'], '2026-01-03 00:00:00+00', '2026-01-03 00:05:00+00',
        'rescore');

select pg_temp.assert_true(
  'freeze captures both plan-anchored candidates',
  public.freeze_trail_rescore_cohort(
    '35000000-0000-0000-0000-000000000021', 'scorePlan@0036', 'bars@0036', '5m'
  ) = 2
);
...
select pg_temp.expect_error(
  ...
  $$insert into public.trail_rescore_expected
    ...
    values ('35000000-0000-0000-0000-000000000021', 'T0036', '5m', 't0036_rule',
      'long', '2026-01-03 00:00:00+00', '35000000-0000-0000-0000-000000000011',
      35001, 'target', 1, 12)$$,
  '42501'
);
select pg_temp.expect_error(
  'service_role direct run update is denied',
  $$update public.trail_rescore_runs set evaluator_version = 'tampered'
     where run_id = '35000000-0000-0000-0000-000000000021'$$,
  '42501'
);
select pg_temp.expect_error(
  'service_role direct run delete is denied',
  $$delete from public.trail_rescore_runs
     where run_id = '35000000-0000-0000-0000-000000000021'$$,
  '42501'
);
select pg_temp.expect_error(
  'service_role direct expected update is denied',
  $$update public.trail_rescore_expected set live_risk_ticks = 99
     where run_id = '35000000-0000-0000-0000-000000000021'$$,
  '42501'
);
select pg_temp.expect_error(
  'service_role direct expected delete is denied',
  $$delete from public.trail_rescore_expected
     where run_id = '35000000-0000-0000-0000-000000000021'$$,
  '42501'
);
reset role;
...
insert into public.opportunity_results (
  ...
  '35000000-0000-0000-0000-000000000021', 'baseline', 'T0036', '5m',
  't0036_rule', 'long', '2026-01-03 00:00:00+00',
  '35000000-0000-0000-0000-000000000011', 35001, true,
  100, 99, 103, 5, 2, 1, 10, 0.25,
  'resolved', 103, 'target', 35099, 1, false,
  12, 2.4, 12, 0, 12, 0, 1, 0, 10, 'scorePlan@0036', 'bars@0036'
);
...
insert into public.opportunity_results (
  ...
  '35000000-0000-0000-0000-000000000021', 'baseline', 'T0036', '5m',
  't0036_rule', 'long', '2026-01-03 00:05:00+00',
  '35000000-0000-0000-0000-000000000012', 35002, false,
  'insufficient_bars', 'skipped', 'scorePlan@0036', 'bars@0036'
);
...
insert into public.opportunity_results (
  ...
  '35000000-0000-0000-0000-000000000021', 'no_trail', 'T0036', '5m',
  't0036_rule', 'long', '2026-01-03 00:10:00+00', null, 35003, true,
  100, 99, 103, 4, 0, 1, 10, 0.25,
  'resolved', 103, 'target', 35099, 1, false,
  12, 3, 12, 0, 12, 0, 1, 0, 10, 'scorePlan@0036', 'bars@0036'
);
...
-- A completed row must not escape the seal by changing its run_id to a run
-- that is still active. This is distinct from mutating a value in place.
insert into public.experiments (id, name, symbols, bars_from, bars_to, kind)
values ('35000000-0000-0000-0000-000000000023', '0036 active move target',
        array['T0036'], '2026-01-03 00:00:00+00', '2026-01-03 00:05:00+00',
        'rescore');
select pg_temp.assert_true(
  'move target has its own frozen cohort',
  public.freeze_trail_rescore_cohort(
    '35000000-0000-0000-0000-000000000023', 'scorePlan@0036', 'bars@0036', '5m'
  ) = 2
);
select pg_temp.expect_error(
  'completed opportunity cannot move to an active run',
  $$update public.opportunity_results
       set run_id = '35000000-0000-0000-0000-000000000023'
     where run_id = '35000000-0000-0000-0000-000000000021'
       and variant = 'baseline' and included$$,
  'P0001', 'sealed after completion'
);
select pg_temp.expect_error(
  'completed rescore experiment cannot be reopened',
  $$update public.experiments set status = 'running'
     where id = '35000000-0000-0000-0000-000000000021'$$,
  'P0001', 'sealed after completion'
);
select pg_temp.expect_error(
  'completed rescore experiment cannot be deleted with its artifacts',
...
select '0036 regression: PASS' as result;
rollback;
