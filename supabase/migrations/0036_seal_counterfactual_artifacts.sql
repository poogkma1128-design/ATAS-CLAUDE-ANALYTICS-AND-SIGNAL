...
-- applies the migration chain once; 0036 is idempotent where PostgreSQL allows
...
create or replace function public.guard_completed_rescore_experiment()
returns trigger
language plpgsql
security invoker
set search_path = ''
as $$
begin
  -- The experiment row controls whether every child artifact is considered
  -- sealed. Reopening it would make the row trigger below stop protecting the
  -- evidence; deleting it would cascade the evidence away. Once done, the
  -- complete rescore record is immutable and a correction needs a new run.
  if old.kind = 'rescore' and old.status = 'done' then
    raise exception using
      errcode = 'P0001',
      message = format('rescore experiment %s is sealed after completion', old.id);
  end if;
...
drop trigger if exists experiments_seal_completed_rescore on public.experiments;
create trigger experiments_seal_completed_rescore
  before update or delete on public.experiments
  for each row execute function public.guard_completed_rescore_experiment();
...
as $$
begin
  -- An UPDATE can try to escape the seal by moving a row from a completed run
  -- to an active one. Check OLD and NEW independently; coalescing them would
  -- inspect only NEW for an UPDATE and leave the completed source unprotected.
  if tg_op in ('UPDATE', 'DELETE') and exists (
    select 1 from public.experiments
     where id = old.run_id and kind = 'rescore' and status = 'done'
  ) then
    raise exception using
      errcode = 'P0001',
      message = format('rescore artifact %s is sealed after completion', old.run_id);
  end if;
  if tg_op in ('INSERT', 'UPDATE') and exists (
    select 1 from public.experiments
     where id = new.run_id and kind = 'rescore' and status = 'done'
  ) then
    raise exception using
      errcode = 'P0001',
      message = format('rescore artifact %s is sealed after completion', new.run_id);
  end if;
  if tg_op = 'DELETE' then
    return old;
  end if;
